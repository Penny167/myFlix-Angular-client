...
export class MovieCardComponent implements OnInit {
  ...
  username = localStorage.getItem('user'); // Username is needed to make the requests to the Api endpoints
  movies: any[] = [];
  favourites: any[] = [];
...
// Fetch the movies and favourites data as soon as the component loads
  ngOnInit(): void { 
    this.getMovies(); 
    this.getFavouriteMovies();
  }
// Fetches the movies and sets the value of the movies property to the response
  getMovies(): void {
    this.fetchApiData.getAllMovies().subscribe((resp: any) => { this.movies = resp });
  }
// Fetches the user and sets the value of the favourites property to the favouriteMovies property on the response
  getFavouriteMovies(): void {
    this.fetchApiData.getUser(this.username!).subscribe((resp: any) => { this.favourites = resp.FavouriteMovies });
  }
// Checks to see if a movie id is included within the array of ids of the user's favourite movies and returns a 
...
// Opens a dialog to display the genre component, passing it the data it needs within the data object
...
// Opens a dialog to display the synopsis component, passing it the data it needs within the data object
...
// Adds the selected movie to the user's favourites
  addMovieToFavourites(movieID: string, title: string): void {
    this.fetchApiData.addFavourite(this.username!, movieID).subscribe((resp: any) => { 
      ...
      this.snackBar.open(`${title} has been added to your favourites!`, 'Cool', { duration: 4000 });
    },  (result) => {
        console.log(result);
...
// Deletes the selected movie from the user's favourites
  deleteMovieFromFavourites(movieID: string, title: string): void {
    this.fetchApiData.deleteFavourite(this.username!, movieID).subscribe((resp: any) => { 
      this.favourites = resp;
      this.snackBar.open(`${title} has been removed from your favourites!`, 'Ok', { duration: 4000 });
    },  (result) => {
        console.log(result);
